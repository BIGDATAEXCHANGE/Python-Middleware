...
**修改链参数：**
    ...
    vi Python-Middleware/PythonMiddlewareBase/chains.py # 编辑链相关参数
	
	```python
	default_prefix = "xxxxxx"
	known_chains = {
    "xxxxxx": {
        "chain_id": "xxxxxx",
        "core_symbol": "xxxxxx",
...
示例：  
```python
pprint(gph.create_account("test3", password="password"))
```
方法：upgrade_account  
...
示例：
```python
print(gph.create_contract("contract.test01", data=data, con_authority="BDES6esv8d6u2eqzKyiQvCYJa6XK74c7BrmzUqL4Z7zfhtvB4dbLh4", account="developer"))
```
...
示例：
```python
pprint(gph.call_order_update(4, "1.3.0", 1, "1.3.1", account="test1"))
```
###### 见证人相关
...
示例：
```python
pprint(gph.create_witness("test2", "", "BDES5YnQru8mtYo9HkckwnuPe8fUcE4LSxdCfVheqBj9fMMK5zwiHb"))
```
方法：approve_witness  
功能：为见证人候选人投票  
参数：  
